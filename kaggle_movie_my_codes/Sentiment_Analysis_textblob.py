...
Y_train = df_train.ix[:155000,['Sentiment']].values.flatten()
...
df_train_cut = df_train.ix[:155000,['Phrase','Sentiment']]
...
def find_best_words(word_scores, number):
    ...
    best_values = sorted(word_scores.items(), key=lambda item:item[1], reverse=True)[:number]
...
X_train_best_words = find_best_words(X_train_word_scores, 2000)
...
print("X_test features concatenate :")
print(length_x_test)

X_test_features = np.hstack((polarity_test_np.reshape((length_x_test,1)),
                              subjectivity_test_np.reshape((length_x_test,1)), 
                              X_test_tfidf_ave.reshape(length_x_test,1), 
                              X_test_w2v_cbow_ave.reshape(length_x_test,1), 
                              X_test_w2v_skip_ave.reshape(length_x_test,1),
                              test_vecs_dm_ave.reshape(length_x_test,1),
                              test_vecs_dbow_ave.reshape(length_x_test,1)))

print("X_test_features's shape is")
print(X_test_features.shape)
#print(X_test_features[:2,:])
...
for c_item in C:
    ...
    test_score = cross_val_score(model_svc, X_train_features_transformed, Y_train, cv=4, scoring='accuracy')
    print("Now C is: %f,test_score is:" %c_item)
    print(test_score)
...
LR_classifier =LogisticRegression(multi_class='multinomial', dual=False, fit_intercept=True,intercept_scaling=1, penalty='l2', random_state=None, tol=0.0001)    

grid_search = GridSearchCV(LR_classifier, parameters,scoring ='accuracy',n_jobs=2,verbose=1)

grid_search.fit(X_train_features_transformed, Y_train)

stop = time.clock()

print('grid_search running time is: ' + str(stop-start) + 's')
...
X_train_data,X_CV_data,Y_train_data,Y_CV_data=train_test_split(X_train_features_transformed,Y_train,test_size=0.1,random_state=42)
 
LR_classifier =LogisticRegression(multi_class='multinomial',solver='newton-cg' ,dual=False, fit_intercept=True,intercept_scaling=1, penalty='l2', random_state=None, tol=0.0001)        
#test_score = cross_val_score(LR_classifier, X_train_features_transformed, Y_train, cv=4, scoring='accuracy')
#print("LR_classifier's test_score are: ",test_score)
#print(np.mean(test_score))    
 
LR_classifier.fit(X_train_data, Y_train_data)
print("LR_classifier's CV accuracy is: ")
print(LR_classifier.score(X_CV_data,Y_CV_data))
test_predict = LR_classifier.predict(X_test_features_transformed)
#make a submission
df_test['Sentiment'] = test_predict.reshape(-1,1) 
header = ['PhraseId', 'Sentiment']
df_test.to_csv('./textblob_LR.csv', columns=header, index=False, header=True)
